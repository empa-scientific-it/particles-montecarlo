...
MaterialPointsFactory::createSimulation(const std::string& fname,
  ...
  // instantiate compute for temperature
  auto temperature = std::make_shared<ComputeTemperature>(timestep, side);

  // set the physical parameters: heat conductivity and specific heat capacity
  temperature->setParams(1.0, 1.0);

  // add the compute to the list of computes
